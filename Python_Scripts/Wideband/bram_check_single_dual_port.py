...
filename = '/home/avanderbyl/fpgs/single_port_bram_test_2019-12-06_1402.fpg'
...
class skarab_debug: 
    ...
    def single_port_debug(self):
        ...
        ss8 = f0.snapshots.snap_cnt8_ss.read(arm=False)['data']
        bram8_in = ss8['bram_in']
        bram8_out = ss8['bram_out']
        dv8_in = ss8['dv_in']
        dv8_out = ss8['dv_out']
        trig8 = ss8['trig']   
        ...
        filename = 'SPBRAM' + '_bram8_in' + '.txt'
        print(filename)
        with open(filename, 'w') as filehandle:
            for listitem in bram8_in:
                filehandle.write('%s\n' % listitem)  
        
        filename = 'SPBRAM' + '_bram8_out' + '.txt'
        print(filename)
        with open(filename, 'w') as filehandle:
        ...
        filename = 'SPBRAM' + '_bram8_inout_Diff' + '.txt'
        print(filename)
        with open(filename, 'w') as filehandle:
            for listitem in Diff:
                filehandle.write('%s\n' % listitem)  
        
        filename = 'SPBRAM' + '_dv8_in' + '.txt'
        print(filename)
        with open(filename, 'w') as filehandle:
            for listitem in dv8_in:
                filehandle.write('%s\n' % listitem)  
        
        filename = 'SPBRAM' + '_dv8_out' + '.txt'
        print(filename)
        with open(filename, 'w') as filehandle:
            for listitem in dv8_out:
                filehandle.write('%s\n' % listitem) 
                
        filename = 'SPBRAM' + '_trig8' + '.txt'
        print(filename)
        with open(filename, 'w') as filehandle:
...
f0.registers.pkt_len.write(len=512)
...
f0.snapshots.snap_cnt8_ss.arm(man_trig=False, man_valid=True)
