...
function cosin_init(blk,varargin)
  ...
  %reuse_block(blk, 'gateway_out1', 'xbsIndex_r4/Constant', 'Position', [295 230 325 230]);
  %add_line(blk,'add_convert1/2', 'gateway_out1/1');
  
%   reuse_block(blk, 'Constant6', 'xbsIndex_r4/Constant', ...
%         'const', '0', ...
%         'arith_type', 'Unsigned', ...
%         'n_bits', '1', ...
%         'bin_pt', '0', ...
%         'explicit_period', 'on', ...
%         'Position', [130 115 175 135]);
%    
%    reuse_block(blk, 'gatewayoutdb', 'xbsIndex_r4/Gateway Out', 'Position', [275 225 335 240]);
%    add_line(blk,'Constant6/1', 'gatewayoutdb/1');
%    
%    reuse_block(blk, 'Terminator5', 'built-in/Terminator', 'Position', [275 225 325 240]);
%    add_line(blk,'gatewayoutdb/1', 'Terminator5/1');

   %reuse_block(blk, 'to', 'xbsIndex_r4/Gateway Out', 'Position', [275 225 335 240]);
  ...
  if strcmp(pack, 'on'),
   ...
   for i = 1:length(coeff_str)
      if strcmp(coeff_str{i},'fft_direct')

           reuse_block(blk, 'gatewayoutdb', 'xbsIndex_r4/Gateway Out', 'Position', [275 225 335 240]);
           add_line(blk,'c_to_ri/1','gatewayoutdb/1');
                
           reuse_block(blk, 'gatewayoutdb1', 'xbsIndex_r4/Gateway Out', 'Position', [275 225 335 240]);
           add_line(blk,'c_to_ri/2','gatewayoutdb1/1');
          
           reuse_block(blk, ['ws' coeff_str{5} '1'], 'Simulink/Sinks/To Workspace', 'VariableName',[coeff_str{5} 'r'],'Position', [275 225 325 240]);
           add_line(blk,'gatewayoutdb/1', ['ws' coeff_str{5} '1' '/1']);

           reuse_block(blk, ['ws' coeff_str{5} '2'], 'Simulink/Sinks/To Workspace', 'VariableName',[coeff_str{5} 'i'],'Position', [275 225 325 240]);
  ...
  elseif strcmp(pack, 'off'),
   ...
   for i = 1:length(coeff_str)
      if strcmp(coeff_str{i},'fft_direct')
           ...
           reuse_block(blk, 'gatewayoutdb', 'xbsIndex_r4/Gateway Out', 'Position', [275 225 335 240]);
           add_line(blk,'lookup/1','gatewayoutdb/1');
                
           reuse_block(blk, 'gatewayoutdb1', 'xbsIndex_r4/Gateway Out', 'Position', [275 225 335 240]);
           add_line(blk,'lookup/2','gatewayoutdb1/1');
