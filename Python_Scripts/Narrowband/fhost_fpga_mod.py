...
class Fengine(object):
    ...
    def delay_set(self, delay_obj):
        ...
        self.last_delay.delay = self._delay_write_delay(delay_obj.delay_whole, delay_obj.delay_frac)
...
#
    def _delay_write_delay(self, delay):
        ...
        reg_bp = delay_reg_frac._fields['initial'].binary_pt
        reg_bw = delay_reg_whole._fields['initial'].width_bits

        max_delay = 2 ** (reg_bw) - 1 / float(2 ** reg_bp)
        if delay < 0:
        ...
        prep_int_frac=int(delay_frac*(2**reg_bp))&((2**(reg_bw+reg_bp))-1)
        act_value_frac = (float(prep_int_frac)/(2**reg_bw))
        
        prep_int_whole=int(delay_whole*(2**reg_bp))&((2**(reg_bw+reg_bp))-1)
        ...
        delay_reg_whole.write_int(prep_int_frac)
    ...
    def _delay_write_delay_rate(self, delay_rate):
        ...
        #figure out register offsets and widths
        reg_bp = delay_delta_reg_frac._fields['delta'].binary_pt
        reg_bw = delay_delta_reg_whole._fields['delta'].width_bits
        ...
        act_value_whole = (float(prep_int_whole)/(2**reg_bp))/bitshift
        
        prep_int_frac=int(delta_delay_shifted_frac*(2**reg_bp))
        act_value_frac = (float(prep_int_frac)/(2**reg_bp))/bitshift
